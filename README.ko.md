...
claudeye로 클로드 코드를 깊이 들여다보세요! 클로드 코드와 Codex의 세션 기록을
읽어 컨텍스트가 어디서 낭비되는지 한 장의 리포트로 보여줍니다. 정적 스크립트로
로컬에서만 동작하며, 개인 데이터를 외부로 내보내지 않습니다.

자기개선 하네스를 꾸린다면 claudeye는 그 계측 계층입니다: 트랜스크립트를 측정된,
에이전트가 읽을 수 있는 사실로 바꿔 회고 루프가 바로 소비하게 합니다.
...
claudeye는 클로드 코드 하네스를 관찰하고, 그 관찰을 하네스 개선의 데이터 원천으로
삼기 위해 시작되었습니다.
...
claudeye의 산출물은 에이전트가 읽기 쉬운 형태라, 계측 → 회고 → 수정 루프로
하네스 자기발전에 활용할 수 있습니다: claudeye가 측정하고, 회고 루틴이
`--data-dir` facet을 읽고, 수정은 스킬·룰·CLAUDE.md에 반영됩니다. 대화에서
교정한 내용을 기록하는 correction-capture 도구가 정성 반쪽이라면, claudeye는
트랜스크립트가 실제로 보여주는 것을 측정하는 정량 반쪽입니다. (이것이
[harness engineering](https://lilianweng.github.io/posts/2026-07-04-harness/)이
말하는 루프입니다 — 측정 가능하고 객관적인 지표, 그리고 파일시스템을 메모리로.)
...
- `volume` — 총 토큰, 신규 지출, cache 재사용, 도구 활동, 피크 데이, 모델 믹스.
- `waste signals` — 도구 오염, 낭비 신호, parse warnings. 값이 있으면 경고색이 되고, 클릭하면 해당 섹션으로 이동합니다.
...
- `Daily tokens` — 날짜별 토큰. 채도=신규 지출, 뮤트=cache 재사용.
- `Tool pollution ranking` — 컨텍스트를 많이 되돌린 도구 순위(결과 바이트).
