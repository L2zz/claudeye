...
def analyze_events(
    ...
    for event in events:
        ...
        # First non-subagent cwd seen is the session's real launch path.
        if stats.cwd is None and event.cwd and not event.agent_id:
            stats.cwd = event.cwd
...
def _fold_assistant(
    ...
    """
    if event.is_api_error:  # the parser already folds SYNTHETIC_MODEL into this
        stats.api_errors += 1
        return
